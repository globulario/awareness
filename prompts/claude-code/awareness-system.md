# Awareness System Prompt — Claude Code
...
When `runtime_status` is `runtime_disabled` in any response, this is expected behavior for projects using NullAdapter (non-Globular projects). Do not attempt to call live cluster tools (`cluster_get_health`, `live_service_health`, etc.). These tools are not available and will fail. This is correct — awareness works without a live cluster.
